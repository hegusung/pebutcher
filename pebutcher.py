            ...
            for item in self.__keys__:
                if name in item:
                    try:
                        ...
                        self.pe.__data__ = self.pe.__data__[:self.get_file_offset()] + new_data + self.pe.__data__[self.get_file_offset()+len(new_data):]
...
class SectionStructure(Structure):
    ...
    def set_data(self, data):
        ...
        self.pe.__data__ = self.pe.__data__[:self.PointerToRawData] + data + self.pe.__data__[self.PointerToRawData+len(data):]
    ...
    def set_raw_data_size(self, size):
        ...
        if max_size and new_section_size > max_size:
            raise PEException("Impossible to increase size")
        ...
        data = b'\x00' * (new_section_size - old_section_size)

        self.pe.__data__ = self.pe.__data__[:self.PointerToRawData + self.SizeOfRawData] + data + self.pe.__data__[self.PointerToRawData + self.SizeOfRawData:]
        ...
        #self.pe.set_dword_at_offset(section_offset+0x10, new_section_size)
        ...
        for index, section in enumerate(self.pe.sections):
            ...
            if section.PointerToRawData > self.PointerToRawData:
                # Increment the value of the pointer to raw data of the next
                # sections
                #print("increment %d" % index)
                #offset = section_table_offset + index*0x28
                #self.pe.set_dword_at_offset(section_offset+0x14, section.PointerToRawData + len(data))
    ...
    def __setattr__(self, name, val):
        ...
        Structure.__setattr__(self, name, val)
...
class PE(object):
    ...
    def add_new_imports(self, entries):
        ...
        self.add_section(".itt", 0xC0000040)

        section = self.sections[-1]
        section_offset = self.sections[-1].VirtualAddress
        ...
        section_data += b'\x00'*(IT_offset-IAT_offset-len(IAT_ILT))
        section_data += IT
        section_data += b'\x00'*(ILT_offset-IT_offset-len(IT))
        section_data += IAT_ILT
        section_data += b'\x00'*(DATA_offset-ILT_offset-len(IAT_ILT))
        section_data += DATA
    ...
    def add_section(self, name, characteristics, data=None):
        ...
        if self.FILE_HEADER.NumberOfSections == len(self.sections):
            ...
            # Define Virtual Address (address of last byte of the last section
            # +1)
            ...
            if (self.sections[-1].Misc_VirtualSize % SectionAlignment) != 0:
                    VirtualAddress = (self.sections[-1].VirtualAddress + self.sections[-1].Misc_VirtualSize - (self.sections[-1].Misc_VirtualSize % SectionAlignment) + SectionAlignment)
    ...
    def __addSectionHeader(self, name, VirtualSize, VirtualAddress, RawSize, RawAddress, characteristics):
        ...
        if new_section_offset + 0x28 < self.OPTIONAL_HEADER.SizeOfHeaders:
            ...
            # Parse the section table of the file again de add the new section
